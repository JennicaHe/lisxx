	...
	<view>
		<view class="exam-item-testing">
			<view class="exam-item-inner" v-if='dataList.length > 0'>
				...
				<!--  单选 -->
				<view v-show="checkShow(0)" style="min-height: 300rpx;">
					<view class="exam-item-option" v-for="(item,rowIndex_1) in currentItem.QuestionOptionList" v-bind:key='rowIndex_1'
					 @click="radioChange(item)">
						<radio :checked="item.fldOptionIndex == currentCheck" color="#C00000"/>
				...
				<view class="exam-button-row">
					<view style="margin-top: 30rpx;">
						<button class="btnsty" hover-class="hbtnsty" v-show="currentIndex != 0" size='mini' @tap="lastQuestion" style="margin-right: 20rpx;">{{lastText}}</button>
						<button class="btnsty" hover-class="hbtnsty" v-show="currentIndex < dataList.length-1"   size='mini' @tap="nextQuestion">{{nextText}}</button>
						<button class="btnsty" hover-class="hbtnsty" size='mini' @tap="finish" style="margin-left: 20rpx;">{{finishText}}</button>
